...
class FIFO资金追踪器:
    ...
    Attributes:
        ...
        累计挪用金额 (float): 累计挪用金额
        累计垫付金额 (float): 累计垫付金额
        累计由资金池回归公司余额本金 (float): 累计由资金池回归公司余额本金
        累计由资金池回归个人余额本金 (float): 累计由资金池回归个人余额本金
        累计非法所得 (float): 累计非法所得
        总计个人应分配利润 (float): 总计个人应分配利润
        总计公司应分配利润 (float): 总计公司应分配利润
    """
    ...
    def __init__(self):
        ...
        self.累计由资金池回归公司余额本金 = 0.0
        # 累计由资金池回归个人余额本金（通过投资产品赎回归还的个人本金）
        self.累计由资金池回归个人余额本金 = 0.0
        # 累计非法所得（投资收益中的非法部分）
        self.累计非法所得 = 0.0
        # 总计个人应分配利润
        self.总计个人应分配利润 = 0.0
    ...
    def 处理投资产品赎回(self, 金额: float, 资金属性: str, 交易日期: Optional[pd.Timestamp]) -> Tuple[float, float, str]:
        ...
        if 总金额 > 0:
            ...
            if 归还的公司本金 > 0:
                ...
                self.累计由资金池回归公司余额本金 = self.格式化数值(self.累计由资金池回归公司余额本金)
            if 归还的个人本金 > 0:
                self.累计由资金池回归个人余额本金 += 归还的个人本金
                self.累计由资金池回归个人余额本金 = self.格式化数值(self.累计由资金池回归个人余额本金)
        else:
            # 资金池为0或负数，纯收益分配
            收益 = 金额
        ...
        if 收益 > 0:
            ...
            # 收益按比例分配，不算作非法所得
            self.总计个人应分配利润 += 个人收益
            self.总计个人应分配利润 = self.格式化数值(self.总计个人应分配利润)
            self.总计公司应分配利润 += 公司收益
            self.总计公司应分配利润 = self.格式化数值(self.总计公司应分配利润)
        else:
            个人收益 = 0
            公司收益 = 0
